...
p(note). optionally, you can ignore certain locations by adding scalate.ignore setting. For example: scalate.ignore=/MyController will render MyController with the standard template engine while everything else is still handled by Scalate. Worth noting that an external module with separate routes like the CRUD module will be rendered with the standard templating language since an external module does not inherit from scalate.Controller

finally you will need to change your controllers to inherit from play.mvc.scalate.Controller



h2. Example

bc. object Application extends scalate.Controller  {
...
h2. Differences between production and dev mode
...
h2. TODO
add reverse url and layout support 
